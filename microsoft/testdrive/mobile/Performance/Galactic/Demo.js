...
//        this.sin = Math.sin(t * orbitSpeed * gSimSpeed * Math.PI / 180);
        var reduce1 = t * orbitSpeed * gSimSpeed * Math.PI / 180
        this.cos = Math.cos(reduce1);
        this.sin = Math.sin(reduce1);
...
//        );
        ...
        mesh.position = new THREE.Vector3(
            this.cos * radius + centerpoint.x,
            centerpoint.y,
            this.sin * radius + centerpoint.z
        );

//        var standardRotation = 1 / 50; //original
        var standardRotation = 0.02;

//        mesh.rotation.x += standardRotation * rotationSpeed.x * gSimSpeed; //rotationSpeed.x = 0 for earth
       // mesh.rotation.y += standardRotation * rotationSpeed.y * gSimSpeed;
        mesh.rotation.y += standardRotation //gSimSpeed =1,rotationSpeed.y =1 for earth
//        mesh.rotation.z += standardRotation * rotationSpeed.z * gSimSpeed; //rotationSpeed.z = 0 for earth
...
//    gScreen.height = 727;
     gScreen.width = 480;
     gScreen.height = 707;//to make room for text
...
//    gCamera1.target.position = new THREE.Vector3(-790, -290, 0);
    ...
    gCamera2.target.position = new THREE.Vector3(0, 100, -1000);
...
//    gMercury = new Planet(8, new THREE.Vector2(8, 8), 180, gGalaxy.CenterPoint, 1.6, new THREE.Vector3(0, 1, 0), Materials.Mercury.Material, 1);
    ...
    gEarth = new Planet(20, new THREE.Vector2(7, 7), 300, gGalaxy.CenterPoint, 1, new THREE.Vector3(0, 1, 0), Materials.Earth.Material, 1);
...
//    AddEntity(gMars);
    ...
    // Sun
     for(var i = 0; i < 5; i++) {
        ...
        ring.rotation = new THREE.Vector3(i * 13, i * 13, 0);
...
function Loop() {
    ...
    //nata
    if (NumSkip % 50 == 0) {
        FPS_text.innerHTML = "FPS= " + gFpsData.AvgFps;
